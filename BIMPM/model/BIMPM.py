...
class BIMPM(nn.Module):
    def __init__(self, args, data):
        ...
        # ----- Matching Layer -----
        for i in range(1, 9):
            setattr(self, 'mp_w'+str(i), nn.Parameter(torch.rand(self.l, self.args.hidden_size)))
    ...
    def forward(self, **kwargs):
        ...
        con_p, last_con_p = self.context_LSTM(p)
        con_h, last_con_h = self.context_LSTM(h)
        ...
        last_con_p_fw, con_p_bw = torch.split(last_con_p, self.args.hidden_size, dim=-1)
        last_con_h_fw, con_h_bw = torch.split(last_con_h, self.args.hidden_size, dim=-1)

        # 1. Self-Attentive-Matching

        # 2. Cross Attentive-Matching
        # (batch, seq_len1, seq_len2)
        ...
        mv_p = torch.cat(
            [mv_p_att_mean_fw, mv_p_att_mean_bw], dim=2)
        mv_h = torch.cat(
            [mv_h_att_mean_fw, mv_h_att_mean_bw], dim=2)
